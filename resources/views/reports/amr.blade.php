...
@section('content')
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">AMR Report</h1>
            <p class="mt-1 text-sm text-gray-500">Aged Milled Rice results — one row per pile</p>
        </div>
        ...
        <table class="min-w-max border-collapse text-sm">
            <thead class="bg-gray-100 text-center text-xs font-semibold uppercase tracking-wide text-gray-700">
                <tr>
                    ...
                    <th rowspan="2" class="border border-gray-300 px-3 py-3">Rice Miller</th>
                    @for ($trial = 1; $trial <= 3; $trial++)
                        <th colspan="3" class="border border-gray-300 px-3 py-3">Trial {{ $trial }}</th>
                    @endfor
                    <th rowspan="2" class="border border-gray-300 px-3 py-3">Pile Action</th>
                </tr>
                <tr>
                    @for ($trial = 1; $trial <= 3; $trial++)
                        <th class="border border-gray-300 px-3 py-2">Palay Input (kg)</th>
                        <th class="border border-gray-300 px-3 py-2">Rice Output (kg)</th>
                        <th class="border border-gray-300 px-3 py-2">Milling Recovery (%)</th>
                    @endfor
                </tr>
            </thead>
            <tbody>
                @forelse ($recordGroups as $records)
                    ...
                    <tr class="text-center text-gray-700 odd:bg-white even:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-3 py-4 text-left">{{ $records->first()->rice_millers }}</td>
                        @for ($trial = 1; $trial <= 3; $trial++)
                            @php($record = $trialRecords->get($trial))
                            <td class="border border-gray-300 px-3 py-4 text-right">{{ $record ? number_format((float) $record->palay_input_kg, 2) : '—' }}</td>
                            ...
                            <td class="border border-gray-300 px-3 py-4 text-right">{{ $record ? number_format($record->milling_recovery_percentage, 2) : '—' }}</td>
                        @endfor
                        <td class="border border-gray-300 px-3 py-4 align-middle">
                            @if ($records->first()->pile)
                                <p class="font-medium">{{ $records->count() }}/3 trials</p>
                                @if ($records->first()->pile->amr_status)
                                    <p class="mt-1 text-xs font-semibold uppercase text-gray-600">{{ $records->first()->pile->amr_status }}</p>
                                @else
                ...
                @empty
                    <tr>
                        <td colspan="18" class="border border-gray-300 px-3 py-10 text-center text-gray-500">No AMR trial records have been entered yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
