...
class PmrCalculationTest extends TestCase
{
    ...
    public function test_normal_3_trial_calculations_without_outliers(): void
    {
        ...
        $this->assertSame(65.5, $result->median);
        ...
        $this->assertSame(65.5, $result->pmrRate);
    ...
    public function test_outlier_identified_and_excluded_from_pmr_calculation(): void
    {
        ...
        $this->assertTrue($result->isOutlierValid);
        $this->assertTrue($result->isCvValid);
        $this->assertSame(65.0, $result->median);
        $this->assertSame(63.7, $result->lowerLimit);
        $this->assertSame(66.3, $result->upperLimit);
        ...
        $this->assertSame(59.0, $trial3['milling_recovery']);
    }
    ...
    public function test_insufficient_valid_trials_when_two_outliers_occur(): void
    {
        ...
        $this->assertSame(
            'Invalid / Requires Re-establishment',
            $result->statusLabel,
        );
        $this->assertStringContainsString(
            'Fewer than 2 valid trials remain',
    ...
    public function test_exactly_plus_minus_two_percent_boundaries_are_valid(): void
    {
        ...
        $this->assertTrue($result->isOutlierValid);
        $this->assertTrue($result->isCvValid);
        $this->assertSame(60.0, $result->median);
        $this->assertSame(58.8, $result->lowerLimit);
        $this->assertSame(61.2, $result->upperLimit);
        $this->assertSame(3, $result->validTrialCount);
        $this->assertSame(0, $result->outlierCount);
        $this->assertSame(60.0, $result->pmrRate);
    ...
    public function test_cv_within_threshold_passes_statistical_requirement(): void
    {
        ...
        $this->assertTrue($result->isValid);
        $this->assertLessThanOrEqual(5.0, $result->coefficientOfVariation);
        $this->assertSame(65.0, $result->pmrRate);
        $this->assertSame('VALID', $result->status);
    }
    ...
    public function test_cv_above_threshold_fails_even_without_outliers(): void
    {
        ...
        $this->assertStringContainsString(
            'exceeds the 5% maximum threshold',
            $result->statusMessage,
        );
    }
    ...
    public function test_historical_legacy_data_flagged_when_more_than_three_trials(): void
    {
        ...
        $this->assertStringContainsString(
            're-established under current 3-trial rules',
            $result->statusMessage,
        );
    }
    ...
    {
        // Case 1: PMR <= 60.0%
        $eval1 = $this->service->evaluateReestablishment(
            pmrRate: 59.5,
            amrRate: 59.0,
        );
        ...
        // Case 2: PMR < AMR
        $eval2 = $this->service->evaluateReestablishment(
            pmrRate: 62.0,
            amrRate: 64.0,
        );
        ...
        $eval3 = $this->service->evaluateReestablishment(
            pmrRate: 67.0,
            amrRate: 62.0,
        );
        ...
        $eval4 = $this->service->evaluateReestablishment(
            pmrRate: 65.0,
            amrRate: 64.0,
        );
    ...
    public function test_incomplete_when_fewer_than_three_trials(): void
    {
        ...
        $this->assertStringContainsString(
            'THREE (3) laboratory milling trials are required',
            $result->statusMessage,
        );
    }
    ...
    public function test_calculate_and_store_for_pile_persists_three_trials_snapshot(): void
    {
        ...
        $this->assertSame(
            'NFA Potential Milling Recovery (PMR)',
            $calcRecord->snapshot['formula'],
        );
        $this->assertSame(3, $calcRecord->snapshot['required_trials']);
    }

    public function test_pmr_report_filters_by_branch_and_warehouse(): void
    {
        ...
        foreach (
            ...
            PmrRecord::factory()->create([
                ...
                'variety' => 'PD',
                'trial_number' => 1,
                'palay_input_kg' => 10000.0,
                'rice_recovery_kg' => 6500.0,
            ]);
        }
    ...
    public function test_pmr_report_displays_three_trials_and_computation_modal(): void
    {
        ...
        $response->assertOk();
        $response->assertSee(
            'data-open-modal="#pmr-calc-modal-'.$pile->id.'"',
            false,
        );
